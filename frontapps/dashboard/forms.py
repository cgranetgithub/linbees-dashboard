...
class MultipleTaskForm(forms.Form):
    startdate = forms.DateField(label=_('start date'))
    enddate   = forms.DateField(label=_('end date'))

    def __init__(self, request, tasks, *args, **kwargs):
        if request.method == 'POST':
            super(MultipleTaskForm, self).__init__(request.POST, *args, **kwargs)
        else:
            super(MultipleTaskForm, self).__init__(*args, **kwargs)
